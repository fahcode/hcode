...
class VisitState {
  ...
  insertStatementCounter(path) {
    if (!COVER_CONFIGS.cover.statement) return;
    ...
    if (!checkOnLine(path.node.loc.start, COVER_CONFIGS.lineNumber)) {
      return;
    }
  ...
  insertBranchCounter(path, branchName, loc) {
    if (!COVER_CONFIGS.cover.branch) return;
...
function coverFunction(path) {
  if (!COVER_CONFIGS.cover.function) return;
...
function checkOnLine(postiton, lineNumber) {
  // Delete redundant data according to the incoming line number parameter
  if (lineNumber) {
    let onLine = false;
    lineNumber.forEach((line, index) => {
      // check line number
      if (line === postiton.line) {
        onLine = true; // del current line

        lineNumber.splice(index, 1);
      }
    });
    return onLine;
  } else {
...
function programVisitor(types, sourceFilePath = 'unknown.js', opts = {}) {
  ...
  COVER_CONFIGS.lineNumber = opts.lineNumber || null;
  COVER_CONFIGS.cover = { ...COVER_CONFIGS.cover,
    ...opts.cover
  };
